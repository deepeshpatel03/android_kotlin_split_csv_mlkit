...
class MainViewModel : ViewModel() {
    ...
    fun export(context: Context) {
        exportMessagesToCsv(context, _processedMessages.value)
    }
    fun exportMessagesToCsv(context: Context, messages: List<MessageData>) {
        ...
        val fileName = "exported_data_$timeStamp.csv"
        ...
        try {
            ...
            csvWriter.close()
            Log.d("selectec","${file.absolutePath}")
    ...
    fun replaceCurrentMatch(replacement: String) {
        ...
        if (newData.containsKey(columnKey)) {
            newData[columnKey] = replacement
